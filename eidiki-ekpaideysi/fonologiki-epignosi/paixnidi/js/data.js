...
Phono.data = {
    ...
    levels: [
        ...
        },
        {
            ...
            games: [
                ...
                // Last on purpose: production (say your own rhyme out
                // loud) is harder than the recognition/oddity games
                // above it, which the child should already be
        ...
        },
        {
            ...
            games: [
                ...
                { id: "findMiddlePhoneme", title: "Βρες τον Μεσαίο Ήχο!", description: "Ποιος είναι ο μεσαίος ήχος;", icon: "🎯" },
                { id: "phonemeDeletion", title: "Σβήσε τον Ήχο!", description: "Τι μένει χωρίς έναν ήχο;", icon: "🧹" },
                // Last on purpose: substitution is the hardest phonemic
                // skill — swap one sound and see what word comes out.
                { id: "phonemeSubstitution", title: "Άλλαξε τον Ήχο!", description: "Τι λέξη γίνεται αν αλλάξεις έναν ήχο;", icon: "🔄" },
            ]
        },
    ],
    ...
    teacherNotes: {
        ...
        rhymeMemory: "Κάθε κάρτα εκφωνείται μόλις γυρίσει, όχι μόνο εικόνα — έτσι το παιδί ακούει τη ρίμα, δεν τη διαβάζει απλώς. Το ταμπλό (6 ζευγάρια από 2 πιθανά, τυχαίο ανά παιχνίδι) έχει επιλεγεί ώστε καμία κατάληξη να μην μπερδεύεται οπτικά/ηχητικά με άλλη στο ίδιο ταμπλό.",
        produceRhyme: "Τελευταία δραστηριότητα του επιπέδου — η παραγωγή είναι πιο δύσκολη από την αναγνώριση των προηγούμενων. Το παιδί ακούει τη λέξη-στόχο και πρέπει να πει μόνο του, προφορικά, μια λέξη που κάνει ρίμα — η εφαρμογή δεν μπορεί να αξιολογήσει προφορική απάντηση, οπότε ακούς εσύ το παιδί και πατάς «✓ Τα κατάφερε» ή «↻ Χρειάζεται βοήθεια». Το «Χρειάζεται βοήθεια» δείχνει πρώτα ΣΕ ΣΕΝΑ (όχι στο παιδί) 2-3 πραγματικά αποδεκτές ρίμες από την ίδια family, ώστε να ξέρεις τι να δεχτείς αν το παιδί πει κάτι διαφορετικό από τις κάρτες· μετά δίνει την κατάληξη τονισμένη και 4 εικόνες για να διαλέξει (η σωστή είναι πάντα από την ίδια family, οι υπόλοιπες 3 από διαφορετικές — ελεγμένο, όχι τυχαίο). Δέξου κάθε σωστή ρίμα, όχι μόνο μία προκαθορισμένη απάντηση.",
        ...
        findInitialPhoneme: "Το παιδί ακούει μια λέξη και πατάει το κουμπί με τον σωστό ΗΧΟ (όχι απλό γράμμα — κάθε κουμπί εκφωνεί τον δικό του ήχο όταν πατιέται, π.χ. «μμμ»). Το κουμπί «🔒 Λέξεις (δάσκαλος)» δείχνει όλες τις λέξεις της συνεδρίας, όπως και στα υπόλοιπα στάδια. Ξεκινά από εξακολουθητικούς ήχους (μ,ν,λ,ρ,σ,φ,θ,χ,ζ,β,γ) και προχωρά σε στιγμιαίους (π,τ,κ,δ).",
        ...
        findFinalPhoneme: "Τελευταία δραστηριότητα του επιπέδου — ο τελικός ήχος είναι πιο δύσκολος από τον αρχικό. Ίδια λογική: το παιδί ακούει τη λέξη και πατάει το κουμπί με τον σωστό ΗΧΟ. Το κουμπί «🔒 Λέξεις (δάσκαλος)» δείχνει όλες τις λέξεις της συνεδρίας. Ξεκινά από τελικά φωνήεντα (πιο εύκολα), μετά το -ς. Εξήγησε στο παιδί ότι ο τελευταίος ήχος μπορεί να είναι και φωνήεν.",
        ...
        // Level 5 — Φωνήματα (Προχωρημένο). Κανόνας Διαφάνειας: κάθε
        // λέξη του επιπέδου είναι φωνημικά διάφανη (Phono.data.phonemesL5,
        // js/phonemes_l5.js) — κανένα δίψηφο/σύμπλεγμα/διπλό γράμμα.
        phonemeSynthesis: "Οι ήχοι μιας λέξης εμφανίζονται οπτικά, έναν-έναν σε φυσαλίδες, και το παιδί τους ενώνει νοερά — δεν υπάρχει εκφώνηση φωνής σε αυτό το στάδιο. Πες εσύ κάθε ήχο δυνατά καθώς εμφανίζεται, με μικρή παύση ανάμεσά τους ανάλογα με το επίπεδο του παιδιού. Ξεκινά από λέξεις 4 ήχων και προχωρά σε 5 και 6.",
        phonemeAnalysis: "Το παιδί χτυπά για κάθε ήχο (φώνημα) μιας λέξης. Δεν υπάρχει εκφώνηση φωνής σε αυτό το στάδιο — πες εσύ τη λέξη αργά, φωνηματικά, πριν ξεκινήσει το παιδί να χτυπά. Το κουμπί δίπλα εναλλάσσει Βοήθεια/Χωρίς βοήθεια: χωρίς βοήθεια, οι τελείες μέτρησης κρύβονται αμέσως μόλις χτυπήσει.",
        elkoninBoxes: "Το παιδί τοποθετεί κάθε ήχο μιας λέξης στο δικό του κουτάκι (κουτάκια Elkonin). Ανάμεσα στα πλακίδια-ήχους υπάρχουν πάντα 1-2 «παγίδες» — ήχοι που δεν ανήκουν καθόλου στη λέξη· αν το παιδί σύρει μία, απλά δεν κουμπώνει σε κανένα κουτάκι. Δεν υπάρχει εκφώνηση φωνής σε αυτό το στάδιο — πες εσύ τη λέξη φωνηματικά, αργά, πριν ξεκινήσει η άσκηση.",
        findMiddlePhoneme: "Το παιδί ακούει τη λέξη και μετά πατάει το κουμπί με τον ΜΕΣΑΙΟ ήχο της — ο πιο δύσκολος να εντοπιστεί, γιατί δεν είναι ούτε πρώτος ούτε τελευταίος. Οι λανθασμένες επιλογές είναι πάντα άλλοι πραγματικοί ήχοι της ίδιας λέξης, ώστε το παιδί να διαλέγει με βάση τη ΘΕΣΗ και όχι απλώς αναγνωρίζοντας έναν ήχο που ακούγεται κάπου στη λέξη.",
        phonemeDeletion: "Το παιδί ακούει την αρχική λέξη και μετά πατάει ένα από τα 4 ηχητικά κουμπιά — κάθε κουμπί, όταν πατιέται, εκφωνεί τη δική του υποψήφια λέξη-απάντηση, το παιδί διαλέγει με το αυτί, όχι διαβάζοντας γράμματα. Το κουμπί δίπλα εναλλάσσει αν αφαιρείται ο ΠΡΩΤΟΣ ή ο ΤΕΛΕΥΤΑΙΟΣ ήχος (ξεκίνα από τον τελευταίο — πιο εύκολος). Το κουμπί «🔒 Λέξεις (δάσκαλος)» δείχνει όλες τις λέξεις της συνεδρίας, τον ήχο που αφαιρείται σε κάθε μία και τη σωστή απάντηση.",
        phonemeSubstitution: "Τελευταία και δυσκολότερη δραστηριότητα του επιπέδου — παραγωγική, όχι αναγνώριση: το παιδί ακούει «Πες [λέξη]. Τώρα άλλαξε τον πρώτο ήχο /x/ σε /y/. Τι λέξη γίνεται;» και απαντάει προφορικά (🎤) — η εφαρμογή δεν μπορεί να αξιολογήσει προφορική απάντηση, οπότε ακούς εσύ το παιδί και πατάς «✓ Τα κατάφερε» ή «↻ Χρειάζεται βοήθεια». Το αποτέλεσμα είναι μερικές φορές υπαρκτή λέξη (π.χ. πόδι→ρόδι) και μερικές φορές «αστεία λέξη» χωρίς νόημα (π.χ. γάτα→«κάτα») — και οι δύο περιπτώσεις είναι αποδεκτές. Το κουμπί «🔒 Απάντηση (δάσκαλος)» δείχνει πάντα τη σωστή λέξη-στόχο και αν είναι υπαρκτή ή αστεία.",
    },
